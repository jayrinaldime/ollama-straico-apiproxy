...
if LAZYBIRD_API_KEY is not None:
    ...
    async def get_lazybird_model_mapping():
        ...
        if (
            ...
            or (voice_model_last_update_dt_lazybird + timedelta(minutes=60))
            <= datetime.now()
        ):
...
async def get_elevenlabs_model_mapping():
    ...
    if (
        ...
        or (voice_model_last_update_dt_elevenlabs + timedelta(minutes=60))
        <= datetime.now()
    ):
...
async def elevenlabs_tts(request: Request):
    ...
    if voice not in ids:
        if voice not in model_mapping:
            voice = os.environ.get("DEFAULT_ELEVENLABS_VOICE", "9BWtsMINqrJLrRacOk9x")
        else:
            voice = model_mapping[voice]
