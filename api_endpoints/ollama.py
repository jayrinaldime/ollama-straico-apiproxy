...
class OllamaPullModel(BaseModel):
    stream: bool | None = True


@app.post("/api/pull")
def ollama_pull(data: OllamaPullModel):
    try:
        is_stream_request = data.stream
    except:
        is_stream_request = True

    if is_stream_request:
        return Response(
            generate_ollama_pull_stream(), content_type="application/x-ndjson"
        )
    else:
...
def ollamagenerate():
    ...
    return Response(
        generate_ollama_stream(request_msg, model), content_type="application/x-ndjson"
    )
